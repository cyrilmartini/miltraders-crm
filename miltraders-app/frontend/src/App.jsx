  ...
  return (
    <div className="topbar" style={{ height: 52, background: "var(--bg0)", borderBottom: "1px solid var(--border)", display: "flex", alignItems: "center", paddingLeft: 28, paddingRight: 24, gap: 16, position: "sticky", top: 0, zIndex: 50, backdropFilter: "blur(12px)" }}>
      ...
      <div style={{ position: "relative" }}>
        <button className="icon-btn" onClick={() => setNotifOpen(o => !o)} style={{ width: 36, height: 36, borderRadius: 2, background: "var(--bg2)", border: "1px solid var(--border)", color: "var(--text2)", fontSize: 18, display: "flex", alignItems: "center", justifyContent: "center", position: "relative" }}>
          🔔
  ...
  return (
    <div>
      ...
      <div className="kpi-grid" style={{ display: "flex", gap: 12, marginBottom: 24, flexWrap: "wrap" }}>
        <KpiCard label="Total Traders" value={totalTraders} sub="Registered" sparkData={[]} delay={0.05} />
        <KpiCard label="Funded Accounts" value={funded} sub="Active" sparkData={[]} color="var(--green)" delay={0.1} />
        <KpiCard label="Pending Reviews" value={pendingReview} sub="Awaiting" sparkData={[]} color="var(--orange)" delay={0.15} />
        <KpiCard label="Pending Payouts" value={pendingPayouts} sub="Requests" sparkData={[]} color="var(--blue)" delay={0.2} />
        <KpiCard label="Total Paid Out" value={"$" + (totalWithdrawn / 1000).toFixed(1) + "k"} sub="All time" sparkData={[]} delay={0.25} />
      </div>
  ...
  return (
    <div className="fade-in">
      ...
      <div style={{ background: "var(--bg1)", border: "1px solid var(--border)", borderRadius: 2, padding: "24px 28px", marginBottom: 20, display: "flex", alignItems: "center", gap: 20 }}>
        <div style={{ width: 48, height: 48, background: "var(--bg3)", border: "1px solid var(--gold)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 22, fontWeight: 700, color: "var(--gold)", flexShrink: 0 }}>
          {trader.name.split(" ").map(n => n[0]).join("")}
        </div>
        <div style={{ flex: 1 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 4 }}>
            <span style={{ fontSize: 22, fontWeight: 700 }}>{trader.name}</span>
            <Badge label={trader.kyc} />
      ...
      {(() => {
        const accts = trader.accounts;
        const activeAccts = accts.filter(a => a.status === "ACTIVE");
        const failedAccts = accts.filter(a => a.status === "FAILED");
        const passedAccts = accts.filter(a => a.status === "PASSED");
        const fundedAccts = accts.filter(a => a.accountCategory === "FUNDED");
        const totalProfit = accts.reduce((s, a) => s + (Number(a.profit) || 0), 0);
        const avgProfit = accts.length > 0 ? totalProfit / accts.length : 0;
        const bestTrade = accts.length > 0 ? Math.max(...accts.map(a => Number(a.profit) || 0)) : 0;
        const worstTrade = accts.length > 0 ? Math.min(...accts.map(a => Number(a.profit) || 0)) : 0;
        const passRate = (passedAccts.length + fundedAccts.filter(a => a.status === "ACTIVE").length);
        const completed = passRate + failedAccts.length;
        const passPercent = completed > 0 ? Math.round((passRate / completed) * 100) : 0;
        const totalDrawdown = accts.reduce((s, a) => s + (Number(a.currentDrawdown) || 0), 0);
        const avgDrawdown = accts.length > 0 ? totalDrawdown / accts.length : 0;
      ...
      {/* Risk Compliance per Active Account */}
      {(() => {
        const activeAccts = trader.accounts.filter(a => a.status === "ACTIVE");
        if (activeAccts.length === 0) return null;
        return (
          <div style={{ background: "var(--bg1)", border: "1px solid var(--border)", padding: "20px 24px", marginBottom: 20, borderRadius: 2 }}>
            <div style={{ fontSize: 15, fontWeight: 600, color: "var(--text2)", marginBottom: 16, letterSpacing: "0.04em", textTransform: "uppercase" }}>Active Account Risk Compliance</div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(300px, 1fr))", gap: 16 }}>
              {activeAccts.map(a => {
                const ddPercent = a.maxDrawdown > 0 ? Math.min(100, Math.round((a.currentDrawdown / a.maxDrawdown) * 100)) : 0;
                const profitPercent = a.profitTarget > 0 ? Math.min(100, Math.round(Math.max(0, a.profit) / a.profitTarget * 100)) : 0;
                const ddColor = ddPercent > 75 ? "var(--red)" : ddPercent > 50 ? "var(--orange)" : "var(--green)";
                return (
      ...
      {tab === "accounts" && (() => {
        const acctFilters = [
          ...
          { id: "CHALLENGE_FAILED", label: "Challenge Failed", count: trader.accounts.filter(a => a.status === "FAILED" && a.accountCategory === "EVAL").length },
          { id: "FUNDED_ACTIVE", label: "Funded Active", count: trader.accounts.filter(a => a.status === "ACTIVE" && a.accountCategory === "FUNDED").length },
          { id: "FUNDED_FAILED", label: "Funded Failed", count: trader.accounts.filter(a => a.status === "FAILED" && a.accountCategory === "FUNDED").length },
        ];
        ...
        else if (acctFilter === "CHALLENGE_FAILED") filtered = trader.accounts.filter(a => a.status === "FAILED" && a.accountCategory === "EVAL");
        else if (acctFilter === "FUNDED_ACTIVE") filtered = trader.accounts.filter(a => a.status === "ACTIVE" && a.accountCategory === "FUNDED");
        else if (acctFilter === "FUNDED_FAILED") filtered = trader.accounts.filter(a => a.status === "FAILED" && a.accountCategory === "FUNDED");
        return (
        <div className="fade-in">
          {/* Account sub-filters */}
          <div style={{ display: "flex", gap: 6, marginBottom: 12, flexWrap: "wrap" }}>
            {acctFilters.map(f => {
              const isFailed = f.id.includes("FAILED");
              const isActive = acctFilter === f.id;
              const activeColor = isFailed ? "var(--red)" : "var(--gold)";
